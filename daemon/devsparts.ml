...
let part_to_dev part =
  let err = ref true in
  let n = ref (String.length part) in
  while !n >= 1 && Char.isdigit (part.[!n-1]) do
    err := false;
    decr n
  done;
  if !err then
...
let part_to_partnum part =
  ...
  let partnum = String.sub part !n (String.length part - !n) in
  int_of_string partnum
