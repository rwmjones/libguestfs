...
let rec list_filesystems () =
  let has_lvm2 = Lazy.force Lvm2.available in
  let has_ldm = Lazy.force Ldm.available in
...
and check_with_vfs_type device =
  ...
  (* A single btrfs device can turn into many volumes. *)
    ...
    let vols = Btrfs.btrfs_subvolume_list device in
     ...
     *)
    let default_volume = Btrfs.btrfs_subvolume_get_default device in
    let vols =
      List.filter (
