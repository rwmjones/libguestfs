...
I<-i live> is used for live conversions, in conjunction with the live
conversion client running on the source.
...
L<virt-p2v(1)>, or use live conversion.
...
Using the L<virt-p2v(1)> tool or live conversion client.
...
Set the input method to I<live>.

Virt-v2v will print a token.  You must start the live conversion
client on the source machine and enter the IP address or hostname of
the conversion server, and the token.  See L</LIVE CONVERSION> below.
...
=head1 LIVE CONVERSION

Virt-v2v supports live conversions, where the source machine stays
running during most of the conversion process.  There is a short
(usually a few minutes) cut-over during which the machine being
converted is unavailable, but this downtime is usually far shorter
than normal offline conversions.

=head2 LIVE: HOW IT WORKS

There are two machines involved in live conversions.  The conversion
server runs S<C<virt-v2v -i live [...]>>.  The source physical or
virtual machine runs the I<live conversion client>.  Note the live
conversion client is installed inside the machine that is being
converted, B<not> on the source hypervisor.

Virt-v2v listens on a TCP port and prints a short random token.  On
the live conversion client the operator must enter the IP address or
hostname of the conversion server (the machine running virt-v2v) and
the token.  (The token has three purposes: It authenticates the source
machine to virt-v2v.  It tells the live conversion client which TCP
port number to use.  And it prevents unintentional mix ups where you
might connect the wrong source machine to the wrong virt-v2v
instance.)

The live conversion client connects to virt-v2v and authenticates with
the token.  It then transfers a copy of the source machine's disk(s).
The source machine can continue running as normal during this time.
If a part of the disk which has already been sent is written to, then
the live conversion client detects this and re-sends it.

  source physical
  or virtual
  machine being                     conversion
  converted                         server
 ┌──────────────┐                  ┌────────────────────┐
 │ live         │                  │ $ virt-v2v -i live │
 │ conversion   │  TCP connection  │ Token: IJKL-1234   │
 │ client   ───────────────────────▶                    │
 └──────────────┘                  └────────────────────┘
            ⇡                          ┊
            └┄┄┄┄┄┄←┄┄┄┄┄←┄┄┄┄┄←┄┄┄┄┄┄┄┘
              operator must copy token
                 in this direction

After nearly all of the blocks of the source disks have been copied,
the live conversion client shuts down the source machine into "single
user mode", sends any remaining disk blocks over, and sends over some
metadata about the source, and finally shuts the source machine down
completely.

On the conversion server, virt-v2v performs conversion on the disk
image (also installing virtio drivers if possible).  At this point the
converted and/or virtualized (for P2V) guest can be moved to the
target hypervisor and started.  For
L<in-place conversions|IN PLACE CONVERSION> the final copy can
usually be avoided.
